...
struct PostCode : sdbusplus::server::object_t<post_code, delete_all>
{
            ...
            [this](sdbusplus::message::message &msg) {
                ...
                auto valPropMap = msgData.find("Value");
                {
                    if (valPropMap != msgData.end())
                    {
                        this->savePostCodes(
                            std::get<
                                std::tuple<uint64_t, std::vector<uint64_t>>>(
                                valPropMap->second));
                    }
                }
            }),
  ...
  private:
    ...
    std::map<uint64_t, std::tuple<uint64_t, std::vector<uint64_t>>> postCodes;
    ...
    void savePostCodes(std::tuple<uint64_t, std::vector<uint64_t>> code);

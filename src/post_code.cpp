...
{
    std::vector<std::tuple<uint64_t, std::vector<uint64_t>>> codesVec;
    ...
    else
    {
        ...
        for (std::pair<uint64_t, std::tuple<uint64_t, std::vector<uint64_t>>>
                 code : codes)
            codesVec.push_back(code.second);
    }
    return codesVec;
...
std::map<uint64_t, std::tuple<uint64_t, std::vector<uint64_t>>>
...
void PostCode::savePostCodes(std::tuple<uint64_t, std::vector<uint64_t>> code)
{
...
bool PostCode::deserializePostCodes(
    const fs::path& path,
    std::map<uint64_t, std::tuple<uint64_t, std::vector<uint64_t>>>& codes)
{
    try
    {
        if (fs::exists(path))

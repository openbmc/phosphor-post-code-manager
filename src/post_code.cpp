...
std::vector<uint64_t> PostCode::getPostCodes(uint16_t index)
{
    ...
    if (currentBootCycleIndex() == index)
        return postCodes;
    deserializePostCodes(fs::path(PostCodeListPath + std::to_string(index)), codes);
    return codes;
}
...
{
    std::ofstream os(path.c_str() + "CurrentBootCycleIndex", std::ios::binary);
    cereal::JSONOutputArchive oarchive(os);
    oarchive(currentBootCycleIndex());

    std::ofstream osPostCodes(path.c_str() + std::to_string(currentBootCycleIndex()), std::ios::binary);
